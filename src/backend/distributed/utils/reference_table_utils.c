...
ReplicateSingleShardTableToAllWorkers(Oid relationId)
{
	...
	/*
	 * ReplicateShardToAllWorkers function opens a transaction at each worker and
	 * replicates given shard to all workers. If a worker already has a healthy replica
	 * of given shart, it skips that worker to prevent copying unnecessary data.
	 */
