											   ...
											   Oid *selectPartitionColumnTableId)
{
	...
	Var *targetSelectVar PG_USED_FOR_ASSERTS_ONLY = NULL;
	...
	foreach(targetEntryCell, query->targetList)
	{
		...
		{
			char *exprTypeDefault = "an expression which is not a plain column";
		...
		/* instead of asserting and failing, give more meaningful errors */
		if (!IsA(subqeryTargetEntry->expr, Var))
		{
			elog(ERROR, "Subquery target entry should be Var");
		}

		targetSelectVar = (Var *) subqeryTargetEntry->expr;
		if (targetSelectVar->vartype != insertPartitionColumn->vartype)
		{
			elog(ERROR, "Target table and the corresponding subquery "
						"target list element should have exactly the same data type");
		}
#endif
		...
		if (!IsPartitionColumnRecursive(subqeryTargetEntry->expr, subquery))
		{
			ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
							...
							errdetail(
								...
								"should exactly match a partition column on the "
